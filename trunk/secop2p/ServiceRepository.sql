...
SELECT `engine_id`, e.`name` AS `engine_name`, e.`host` AS `host`, e.`port` AS `port`, e.`desc` AS `engine_desc`, `service_id`, s.`name` AS `service_name`
FROM `services` s INNER JOIN `service_map` ON `service_id` = s.`id` INNER JOIN `engines` e ON e.`id` = `engine_id` 
